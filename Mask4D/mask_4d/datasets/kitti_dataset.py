...
def find_image_for_scan(velodyne_file):
    """Map .../sequences/SS/velodyne/NNNN.bin -> .../image_2/NNNN.<ext> if it exists."""
    seq_dir = os.path.dirname(os.path.dirname(velodyne_file))
    stem = os.path.splitext(os.path.basename(velodyne_file))[0]
...
class SemanticDataset(Dataset):
    def __init__(self, data_path, cfg_path, split="train"):
        ...
        # Per-sequence LiDAR->image projection matrices (4x4) for multimodal fusion.
        self.proj_T = {}
        for cf in calib_files:
            seq = os.path.basename(os.path.dirname(cf))
            try:
                self.proj_T[seq] = proj.projection_matrix(proj.parse_calibration(cf))
            except Exception:
                self.proj_T[seq] = None
    ...
    def __getitem__(self, index):
        ...
        seq = os.path.basename(os.path.dirname(os.path.dirname(fname)))
        img_path = find_image_for_scan(fname)
        proj_T = self.proj_T.get(seq, None)

        return (xyz, sem_labels, ins_labels, intensity, fname, pose, img_path, proj_T)
...
class MaskSemanticDataset(Dataset):
    def __init__(
        ...
        # Image / multimodal config
        img_cfg = img_cfg or {}
        self.use_image = img_cfg.get("ENABLED", True)
        self.img_short_side = img_cfg.get("SHORT_SIDE", 480)
        self.img_crop_ratio = img_cfg.get("CROP_RATIO", 0.7)
        self.img_color_aug = img_cfg.get("COLOR_AUG", True)

    def __len__(self):
        return len(self.dataset)

    def _load_image(self, img_path, T):
        """Load + augment image, returning (img_tensor[3,H,W] RGB in [0,1], T_adj, (H,W)).

        Returns (empty tensor, None, None) when no image/calib is available so the
        model transparently falls back to the LiDAR-only path.
        """
        if not self.use_image or img_path is None or T is None:
            return torch.zeros(0), None, None
        ...
        # 1) Resize so the shorter side == img_short_side (aspect preserving).
        scale = self.img_short_side / min(h0, w0)
        new_w, new_h = int(round(w0 * scale)), int(round(h0 * scale))
        rgb = cv2.resize(rgb, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
        T = proj.adjust_projection_for_resize(T, new_w / w0, new_h / h0)

        # 2) Train-time random crop to crop_ratio of the resized image.
        if self.split == "train" and self.img_crop_ratio < 1.0:
            ch, cw = int(new_h * self.img_crop_ratio), int(new_w * self.img_crop_ratio)
            y0 = random.randint(0, new_h - ch)
            x0 = random.randint(0, new_w - cw)
            rgb = rgb[y0 : y0 + ch, x0 : x0 + cw]
            T = proj.adjust_projection_for_crop(T, x0, y0)

        # 3) Train-time SSD-style color augmentation.
        if self.split == "train" and self.img_color_aug:
            rgb = _color_jitter(rgb)

        img = torch.from_numpy(rgb.astype(np.float32) / 255.0).permute(2, 0, 1)
        return img, T, (img.shape[1], img.shape[2])

    def __getitem__(self, index):
        data = self.dataset[index]
        xyz, sem_labels, ins_labels, intensity, fname, pose, img_path, proj_T = data
        image, proj_T, img_hw = self._load_image(img_path, proj_T)
...
def _color_jitter(rgb):
    """Light SSD-style photometric augmentation on a uint8 RGB image."""
    img = rgb.astype(np.float32)
    # random brightness
    img += random.uniform(-20, 20)
    # random contrast
    img = (img - 128.0) * random.uniform(0.8, 1.2) + 128.0
    # random per-channel saturation-ish jitter
    img *= np.array(
        [random.uniform(0.9, 1.1) for _ in range(3)], dtype=np.float32
    )[None, None, :]
