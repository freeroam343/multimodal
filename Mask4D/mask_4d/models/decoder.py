...
# Modified from Mask2Former; extended for 4D-Former multimodal cross-attention.
...
class MaskedTransformerDecoder(nn.Module):
    def __init__(self, cfg, bb_cfg, data_cfg):
        ...
        # 4D-Former: explicit depth component in the positional encoding.
        self.use_depth_pe = cfg.get("DEPTH_PE", True)
        if self.use_depth_pe:
            self.depth_layer = DepthEncoder(cfg.POS_ENC)
        # 4D-Former: queries also cross-attend to image features (per block).
        self.use_image = cfg.get("IMAGE_CROSS_ATTN", True)
        ...
        self.transformer_image_cross_attention_layers = nn.ModuleList()
        ...
        for _ in range(self.num_layers):
            ...
            if self.use_image:
                self.transformer_image_cross_attention_layers.append(
                    blocks.CrossAttentionLayer(hidden_dim, self.nheads)
                )
    ...
    def _pos_enc(self, coors):
        """Positional encoding: Fourier(xyz) (+ depth component, 4D-Former)."""
        enc = self.pe_layer(coors)
        if self.use_depth_pe:
            enc = enc + self.depth_layer(coors)
        return enc
    ...
    def forward(
        ...
        img_feats=None,
        img_valid=None,
        mask_feats_override=None,
    ):
        """
        img_feats: (1, N, D) per-point image features (sampled from I4+I8 at the
            projected pixel of each point), aligned with `coors`. None disables
            image cross-attention for this scan (e.g. no camera coverage).
        img_valid: (1, N) bool, True where the point has a valid image
            projection. Points without coverage are masked out as keys.
        mask_feats_override: (N, D) fused per-point features Z from point-level
            fusion, used as the mask features instead of mask_feat_proj(feats).
        """
        if mask_feats_override is not None:
            mask_features = mask_feats_override + self._pos_enc(coors)
        else:
            mask_features = self.mask_feat_proj(feats) + self._pos_enc(coors)
        src = []
        pos = []
        queries = []

        use_img = self.use_image and img_feats is not None
        # key_padding_mask for image cross-attn: True = ignore (no projection).
        img_key_pad = None
        if use_img and img_valid is not None:
            img_key_pad = ~img_valid  # (1, N)

        for i in range(self.num_feature_levels):
            pos.append(self._pos_enc(coors))
        ...
        for i in range(self.num_layers):
            ...
            # 4D-Former: cross-attend to image features (global context).
            if use_img:
                output = self.transformer_image_cross_attention_layers[i](
                    output,
                    img_feats,
                    attn_mask=None,
                    padding_mask=img_key_pad,
                    pos=pos[level_index],
                    query_pos=query_embed,
                )
