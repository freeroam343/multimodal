...
from mask_4d.models.tam import TrackletAssociationModule, TrackMemoryBank
...
class Mask4D(LightningModule):
    def __init__(self, hparams):
        ...
        # --- 4D-Former multimodal components ---
        hidden_dim = hparams.DECODER.HIDDEN_DIM
        img_cfg = hparams.get("IMAGE", {})
        self.use_image = img_cfg.get("ENABLED", True)
        if self.use_image:
            self.image_backbone = ResNetFPN(
                out_dim=hidden_dim,
                pretrained=img_cfg.get("PRETRAINED", True),
                freeze_bn=img_cfg.get("FREEZE_BN", True),
            )
            # Project backbone point features to the hidden dim to form Z, then
            # apply point-level fusion of Z with I4 (Sec 3.1).
            self.point_embed = nn.Linear(hparams.BACKBONE.CHANNELS[0], hidden_dim)
            self.point_fusion = PointImageFusion(hidden_dim, img_stride=4)
            self.pf_weight = hparams.LOSS.get("W_PF", 1.0)
            self.img_norm_mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
            self.img_norm_std = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)
        # Tracklet Association Module (Sec 3.3), trained in stage 2.
        tam_cfg = hparams.get("TAM", {})
        self.use_tam = tam_cfg.get("ENABLED", True)
        if self.use_tam:
            self.tam = TrackletAssociationModule(hidden_dim)
            self.tam_memory = TrackMemoryBank(t_hist=tam_cfg.get("T_HIST", 4))
    ...
    def _run_image_backbone(self, image, device):
        """image: (3,H,W) tensor in [0,1]. Returns (fmaps, (H,W))."""
        img = image.unsqueeze(0).to(device).float()
        if img.max() > 1.5:  # came in as 0-255
            img = img / 255.0
        img = (img - self.img_norm_mean.to(device)) / self.img_norm_std.to(device)
        fmaps = self.image_backbone(img)
        return fmaps, image.shape[-2:]

    def _build_point_image_feats(self, xyz, fmaps, T, hw):
        """Per-point image features for decoder cross-attention.

        Samples I4 and I8 at each point's projected pixel and sums them.
        Returns (img_feats [1,N,D], img_valid [1,N]).
        """
        h, w = hw
        uv, depth = proj.project_points(xyz, T)
        valid = proj.valid_projection_mask(uv, depth, w, h)
        f4 = proj.sample_feature_map(fmaps["s4"], uv, 4)
        f8 = proj.sample_feature_map(fmaps["s8"], uv, 8)
        feats = f4 + f8  # (N, D)
        feats = feats * valid.unsqueeze(-1).to(feats.dtype)
        return feats.unsqueeze(0), valid.unsqueeze(0)

    def _has_image(self, x):
        return (
            self.use_image
            and "image" in x
            and len(x["image"]) > 0
            and x["image"][0] is not None
            and (not torch.is_tensor(x["image"][0]) or x["image"][0].numel() > 0)
        )
    ...
    def forward(self, x, track_ins):
        ...
        img_valid = None
        mask_override = None
        pf_loss = feats.new_zeros(())

        if self._has_image(x):
            device = feats.device
            fmaps, hw = self._run_image_backbone(x["image"][0], device)
            T = x["proj"][0]
            xyz = coors[0]
            # Point-level fusion -> enriched per-point features Z (Eq. 1, Eq. 4).
            z = self.point_embed(feats)
            z, pf_loss = self.point_fusion(z, xyz, fmaps["s4"], T, hw)
            mask_override = z
            # Per-point image features for query<->image cross-attention.
            img_feats, img_valid = self._build_point_image_feats(xyz, fmaps, T, hw)

        outputs = self.decoder(
            feats, coors, track_ins, img_feats, img_valid, mask_override
        )
        outputs["pf_loss"] = pf_loss
        return outputs, logits

    # ------------------------------------------------------------------
    # Stage-2: Tracklet Association Module (TAM) training (Sec. 3.3 / 3.4)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def extract_clip_tracklets(self, x):
        """Run the (frozen) stage-1 network over a clip and extract tracklets.

        Returns a list of dicts: {query (D,), center (3,), gt_id int, frame int}
        for each thing-instance tracklet matched to a ground-truth object.
        """
        tracklets = []
        track_ins = self.init_tracks()
        for s_i in range(len(x["pt_coord"])):
            scan = {k: [v[s_i]] for k, v in x.items()}
            outputs, _ = self.forward(scan, track_ins)
            ...
            if targets["masks"][0].shape[0] == 0:
                track_ins = self._roll_tracks(track_ins)
                continue
            indices = self.matcher(outputs, targets)
            pred_idx, tgt_idx = indices[0]
            queries = outputs["queries"][0]
            masks = outputs["pred_masks"][0].sigmoid()
            points = torch.from_numpy(scan["pt_coord"][0]).to(masks.device)
            gt_ids = targets["id"][0]
            frame = int(scan["fname"][0].split("/")[-1][:-4])
            for p, t in zip(pred_idx.tolist(), tgt_idx.tolist()):
                gid = int(gt_ids[t])
                if gid == 0:  # stuff segment, not a track
                    continue
                m = masks[:, p] >= 0.5
                if m.sum() < self.min_mask_pts:
                    continue
                center = misc.center_from_points(misc.remove_outliers(points[m]))
                tracklets.append(
                    {
                        "query": queries[p].detach(),
                        "center": torch.as_tensor(
                            center, device=masks.device, dtype=torch.float32
                        ),
                        "gt_id": gid,
                        "frame": frame,
                    }
    ...
    def _roll_tracks(self, track_ins):
        new_track_ins = self.init_tracks()
        return Tracks.cat([new_track_ins, track_ins])

    def tam_loss_from_tracklets(self, tracklets):
        """BCE association loss over all tracklet pairs across frames in a clip."""
        if len(tracklets) < 2:
            return None
        a, b = [], []
        for i in range(len(tracklets)):
            for j in range(i + 1, len(tracklets)):
                if tracklets[i]["frame"] == tracklets[j]["frame"]:
                    continue  # only associate across frames
                a.append(i)
                b.append(j)
        if not a:
            return None
        device = tracklets[0]["query"].device
        q_a = torch.stack([tracklets[i]["query"] for i in a])
        c_a = torch.stack([tracklets[i]["center"] for i in a])
        q_b = torch.stack([tracklets[i]["query"] for i in b])
        c_b = torch.stack([tracklets[i]["center"] for i in b])
        gap = torch.tensor(
            [abs(tracklets[i]["frame"] - tracklets[j]["frame"]) for i, j in zip(a, b)],
            device=device,
        )
        iou = torch.zeros(len(a), device=device)  # no overlap across distinct frames
        labels = torch.tensor(
            [
                1.0 if tracklets[i]["gt_id"] == tracklets[j]["gt_id"] else 0.0
                for i, j in zip(a, b)
            ],
            device=device,
        )
        logits = self.tam(q_a, c_a, q_b, c_b, gap, iou)
    ...
    def training_step(self, x: dict, idx):
        ...
        for s_i in range(len(x["pt_coord"])):
            ...
            # 4D-Former pseudo-fusion loss (Eq. 4)
            if self.use_image and "pf_loss" in outputs:
                losses[f"pf_{s_i}"] = self.pf_weight * outputs["pf_loss"]
